...
use notify::{self, event::CreateKind, EventKind, RecursiveMode, Watcher};
...
fn main() {
    ...
    thread::spawn(move || {
        if let Ok(mut signals) = Signals::new(&[SIGINT, SIGHUP, SIGTERM]) {
            ...
            for sig in signals.forever() {
                ...
                {
                    let mut s = sig_stop.lock().unwrap();
                    *s = true;
                }
    ...
    let notify_handle = thread::spawn(move || {
        ...
        if let Ok(mut watcher) = notify::PollWatcher::new(not_tx, notify::Config::default().with_poll_interval(Duration::from_secs(60))) {
                                    ...
                                    CreateKind::Any => {
                                        ...
                                        for p in &event.paths {
                                            if filters.iter().all(|f| f.filter(Path::new(p))) {
                                                ...
                                                if let Err(send_error) = notify_wq_tx.send(PathBuf::from(p)) {
                                                    println!("{} notify_wq_tx.send() error: {:?}", now_string(), send_error);
                                                }
                                            }
                                        }
                                    },
    ...
    process_directory(Arc::clone(&stop), &PathBuf::from(&args[1]), wq_tx.clone(), &filters);

    let _ = wq_handle.join();
    let _ = notify_handle.join();
}

fn process_directory(stop: Arc<Mutex<bool>>, path: &PathBuf, tx: Sender<PathBuf>, filters: &Vec<Box<dyn PathFilter>>) {
    if let Ok(rd) = fs::read_dir(path) {
        ...
        for entry in entries {
            ...
            if let Ok(metadata) = entry.metadata() {
                if metadata.is_dir() {
                    process_directory(Arc::clone(&stop), &entry.path(), tx.clone(), filters);
                } else {
                    let p = entry.path();
                    if filters.iter().all(|f| f.filter(Path::new(&p))) {
                        println!("{} enqueue {:?}", now_string(), &p); 
